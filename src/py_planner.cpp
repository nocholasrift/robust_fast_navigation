...
PYBIND11_MODULE(py_planner, m)
{
    ...
    py::class_<planner_params>(m, "PlannerParams")
        ...
        .def_readwrite("DT_FACTOR_INCREMENT", &planner_params::DT_FACTOR_INCREMENT)
        .def_readwrite("SOLVER_TRAJ_DT", &planner_params::SOLVER_TRAJ_DT)
        .def_readwrite("W_MAX", &planner_params::W_MAX)
        .def_readwrite("FORCE_FINAL_CONSTRAINT", &planner_params::FORCE_FINAL_CONSTRAINT)
    ...
    py::class_<map_util::OccupancyGrid>(m, "OccupancyGrid", py::module_local())
        .def(py::init<>())
        .def(py::init<int, int, double, double, double, std::vector<unsigned char> &,
                      const std::vector<unsigned char> &,
                      const std::vector<unsigned char> &>())
        .def("is_occupied", py::overload_cast<double, double, map_util::Layer>(
        ...
        .def("update",
             py::overload_cast<
                 int, int, double, double, double, const std::vector<unsigned char> &,
                 const std::vector<unsigned char> &, const std::vector<unsigned char> &, int>(
