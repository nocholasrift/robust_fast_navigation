...
bool JPSPlan::add_to_parents(const JPSNode_t &node, const JPSNode_t &parent,
                             double cost) {
...
bool JPSPlan::explore_diagonal(const JPSNode_t &start) {
  ...
  while (true) {
    ...
    if (n.x > sizeX - 1)
      std::cout << "[JPS] DIAGONAL AHHHHHH: " << sizeX - 1 << " " << n.x
                << std::endl;
    if (n.y > sizeY - 1)
      std::cout << "[JPS] DIAGONAL AHHHHHH: " << sizeY - 1 << " " << n.y
                << std::endl;
...
int JPSPlan::JPS() {
  ...
  if (_map_util.is_occupied((unsigned int)startX, (unsigned int)startY,
                            _layer)) {
...
***********************************************************************/
void JPSPlan::set_map(const std::vector<unsigned char> &map, int sizeX,
                      int sizeY, double originX, double originY,
                      double resolution) {
  this->_map = map;
  this->sizeX = sizeX;
  this->sizeY = sizeY;
...
void JPSPlan::set_util(const map_util::occupancy_grid_t &map,
                       map_util::Layer layer) {
  _map_util = map;
...
***********************************************************************/
bool JPSPlan::isBlocked(const Eigen::Vector2d &p1, const Eigen::Vector2d &p2,
  ...
  double rayx, rayy;
  return _map_util.raycast(sx, sy, ex, ey, rayx, rayy, _layer);
  ...
  /*double scale        = (dist == 0.0) ? 1.0 : std::min(1.0, max_range /
   * dist);*/
  /**/
  /*unsigned int term;*/
  /*if (abs_dx >= abs_dy)*/
...
***********************************************************************/
bool JPSPlan::bresenham(unsigned int abs_da, unsigned int abs_db, int error_b,
...
bool JPSPlan::worldToMap(double x, double y, unsigned int &mx,
                         unsigned int &my) {
...
void JPSPlan::mapToWorld(unsigned int mx, unsigned int my, double &x,
                         double &y) {
