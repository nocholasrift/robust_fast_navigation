...
GLEVec::GLEVec(std::initializer_list<GRBLinExpr> ilist)
{
    x.reserve(ilist.size());
    for (auto it = ilist.begin(); it != ilist.end(); it++)
    {
...
GLEVec GLEVec::operator+(const GVec &rhs) const
{
    if (rhs.x.size() != x.size() || rhs.x.size() == 0 || x.size() == 0)
    {
        throw std::runtime_error("GLEVec: size mismatch " + std::to_string(rhs.x.size()) +
                                 " != " + std::to_string(x.size()));
    }
...
GLEVec GLEVec::operator+(const GLEVec &rhs) const
{
    if (rhs.x.size() != x.size() || rhs.x.size() == 0 || x.size() == 0)
    {
        throw std::runtime_error("GLEVec: size mismatch " + std::to_string(rhs.x.size()) +
...
/*GLEVec GLEVec::operator*(double rhs) const*/
/*{*/
/*    GLEVec ret;*/
/*    ret.x.reserve(x.size());*/
/*    for (int i = 0; i < x.size(); ++i)*/
...
GLEVec operator*(const GLEVec &lhs, double rhs)
{
    GLEVec ret;
    ret.x.reserve(lhs.x.size());
    for (int i = 0; i < lhs.x.size(); ++i)
...
GLEVec operator*(double lhs, const GLEVec &rhs)
{
    GLEVec ret;
    ret.x.reserve(rhs.x.size());
    for (int i = 0; i < rhs.x.size(); ++i)
...
GLEVec operator*(const Eigen::MatrixX3d &lhs, const GLEVec &rhs)
{
    if (lhs.cols() != rhs.x.size() || rhs.x.size() == 0)
    {
        throw std::runtime_error("GLEVec: size mismatch " + std::to_string(lhs.cols()) +
...
GRBQuadExpr GLEVec::squaredNorm()
{
    GRBQuadExpr norm = 0;
    for (int i = 0; i < x.size(); ++i)
    {
