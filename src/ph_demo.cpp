...
public:
  AtomicSignedDistance(const Eigen::MatrixXd &sdf, double resolution,
      ...
      : CppAD::atomic_three<double>("SignedDistance"), sdf_(sdf),
        resolution_(resolution), origin_x_(origin[0]), origin_y_(origin[1]) {}
  ...
  bool reverse(const CppAD::vector<double> &parameter_x,
               const CppAD::vector<CppAD::ad_type_enum> &type_x,
               size_t order_up, const CppAD::vector<double> &taylor_x,
  ...
  double interpolate(double x, double y) const {
    ...
    /**/
    /*return (1 - dx) * (1 - dy) * v00 + dx * (1 - dy) * v10 + (1 - dx) * dy *
     * v01 +*/
    /*       dx * dy * v11;*/
  }
};
...
public:
  ...
  void setMap(const map_util::occupancy_grid_t &map) {
    atomic_sdf_ = std::make_shared<AtomicSignedDistance>(
        map._sdf, map.get_resolution(), map.get_origin());
  }
  ...
  void operator()(ADvector &fg, const ADvector &x) {
    ...
    /*output[0] =
     * atomic_sdf_->interpolate(CppAD::Value(CppAD::Var2Par(p1[0])),*/
    ...
    std::cout << "goal: " << traj_.back().getGoal()[0]
              << traj_.back().getGoal()[1] << std::endl;
    ...
    for (int i = 0; i < traj_.numSegments() - 1; ++i) {
      ...
      fg[offset++] = v_const[0];
      fg[offset++] = v_const[1];
      std::cout << "vel residual: " << v_const[0] << ", " << v_const[1]
                << std::endl;
    }
    ...
    for (int i = 0; i < traj_.numSegments() - 1; ++i) {
      ...
      ADVec2 a_const =
          4 * ((traj_[i].getV4(xi) - traj_[i].getV3(xi)) -
               (traj_[i + 1].getV1(xi1) - traj_[i + 1].getV0(xi1)));
      fg[offset++] = a_const[0];
      fg[offset++] = a_const[1];

      std::cout << "acc residual: " << a_const[0] << ", " << a_const[1]
                << std::endl;
    }
    ...
    /**/
    /*    std::vector<ADVec2> points = {traj_[i].getP1(xi),
     * traj_[i].getP2(xi),*/
    /*                                  traj_[i].getP3(xi),
     * traj_[i].getP4(xi),*/
    /*                                  traj_[i].getP5(xi)};*/
    ...
    /*                AD<double> penalty = CppAD::log(CppAD::exp(slack / eps)
     * + 1);*/
...
PHSolver::PHSolver() : n_segments_(0) {
  ph_ =
      std::make_unique<PlanarTrajectory<CppAD::AD<double>,
...
void PHSolver::setPolys(const std::vector<Eigen::MatrixX3d> &hpolys) {
  hpolys_ = hpolys;
}
...
bool PHSolver::solve(const std::vector<double> &x0) {
  ...
  AtomicSignedDistance atomic_sdf(map_._sdf, map_.get_resolution(),
                                  map_.get_origin());
  ...
  ok &= (solution.status == CppAD::ipopt::solve_result<Dvector>::success ||
         ...
         solution.status ==
             CppAD::ipopt::solve_result<Dvector>::feasible_point_found);
...
std::vector<double> PHSolver::warm_start() {
  ...
  /*auto *x1polycost = new
   * ceres::DynamicAutoDiffCostFunction<PolytopeResidualStart, 4>(*/
  ...
  if (hpolys_.size() > 0) {
    ...
    auto *x2polycost =
        new ceres::DynamicAutoDiffCostFunction<PolytopeResidualInterior, 4>(
  ...
  /*std::cout << "u = [" << xs[0] << ", " << xs[1] << ", " << xs[2] << "]" <<
   * std::endl;*/
  /*std::cout << "v = [" << xs[3] << ", " << xs[4] << ", " << xs[5] << "]" <<
   * std::endl;*/
  /**/
  /*std::cout << "u = [" << xs[6] << ", " << xs[7] << ", " << xs[8] << "]" <<
   * std::endl;*/
  /*std::cout << "v = [" << xs[9] << ", " << xs[10] << ", " << xs[11] << "]"
   * << std::endl;*/
...
const PlanarTrajectory<AD<double>, CPPAD_TESTVECTOR(CppAD::AD<double>)> &
PHSolver::getTraj() const {
  return *ph_;
}
