...
void SolverGurobi::ResetToNormalState() { cb_.should_terminate_ = false; }
...
SolverGurobi::SolverGurobi() {
  ...
  m.setCallback(
      &cb_); // The callback will be called periodically along the optimization
}
...
void SolverGurobi::setN(int N) { N_ = N; }

void SolverGurobi::setMode(int mode) { mode_ = mode; }

void SolverGurobi::createVars() {
  std::vector<std::string> coeff = {"ax", "ay", "az", "bx", "by", "bz",
                                    "cx", "cy", "cz", "dx", "dy", "dz"};
  ...
  for (int t = 0; t < N_; t++) {
    ...
    for (int i = 0; i < 12; i++) {
      row_t.push_back(m.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_CONTINUOUS,
                               coeff[i] + std::to_string(t)));
    }
    x.push_back(row_t);
  }
}

void SolverGurobi::setObjective() // I need to set it every time, because the
                                  // objective depends on the xFinal
{
  ...
  for (int t = 0; t < N_; t++) {
    std::vector<GRBLinExpr> ut = {getJerk(t, 0, 0), getJerk(t, 0, 1),
                                  getJerk(t, 0, 2)};
    ...
    if (is_occ_) {
      ...
      std::vector<GRBLinExpr> cp0_r = {cp0[0] - p[0], cp0[1] - p[1],
                                       cp0[2] - p[2]};
      std::vector<GRBLinExpr> cp1_r = {cp1[0] - p[0], cp1[1] - p[1],
                                       cp1[2] - p[2]};
      std::vector<GRBLinExpr> cp2_r = {cp2[0] - p[0], cp2[1] - p[1],
                                       cp2[2] - p[2]};
      std::vector<GRBLinExpr> cp3_r = {cp3[0] - p[0], cp3[1] - p[1],
                                       cp3[2] - p[2]};
  ...
  // m.setObjective(control_cost + final_state_cost + distance_to_JPS_cost,
  // GRB_MINIMIZE);
...
void SolverGurobi::fillX() {
  ...
  // Force the final input to be 0 (I'll keep applying this input if when I
  // arrive to the final state I still haven't planned again).
...
void SolverGurobi::setPolytopes(
    const std::vector<Eigen::MatrixX4d> &polytopes) {
...
void SolverGurobi::setPolytopesConstraints() {
  ...
  // Remove previous binary variables  (They depend on the number of
  // polytopes--> I can't reuse them)
  ...
  if (polytopes_.size() > 0) // If there are polytope constraints
  {
    ...
    for (int t = 0; t < N_ + 1; t++) {
      ...
      {
        GRBVar variable =
            m.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_BINARY,
    ...
    // Polytope constraints (if binary_varible==1 --> In that polytope) and
    // at_least_1_pol_cons (at least one polytope) constraints
    for (int t = 0; t < N_; t++) // Start in t=1 (because t=0 is already fixed
                                 // with the initial condition)
    {
      ...
      at_least_1_pol_cons.push_back(m.addConstr(
          sum == 1, "At_least_1_pol_t_" + std::to_string(t))); // at least in
                                                               // one polytope
      ...
      if (use_minvo_) {
        ...
        std::vector<std::vector<GRBLinExpr>> cps_minvo = Multiply2Matrices(
            Multiply2Matrices(cps, bezier_std), minvo_inv_std);
      ...
      for (int n_poly = 0; n_poly < polytopes_.size();
           n_poly++) // Loop over the number of polytopes
      {
        // Constraint A1x<=b1
        ...
        std::vector<GRBLinExpr> Acp0 =
            MatrixMultiply(A1std, cp0); // A times control point 0
        std::vector<GRBLinExpr> Acp1 =
            MatrixMultiply(A1std, cp1); // A times control point 1
        std::vector<GRBLinExpr> Acp2 =
            MatrixMultiply(A1std, cp2); // A times control point 2
        std::vector<GRBLinExpr> Acp3 =
            MatrixMultiply(A1std, cp3); // A times control point 3

        for (int i = 0; i < bb.rows(); i++) {
          std::string name0 = "Poly" + std::to_string(n_poly) + "_face" +
                              std::to_string(i) + "_t" + std::to_string(t) +
                              "_cp0";
          std::string name1 = "Poly" + std::to_string(n_poly) + "_face" +
                              std::to_string(i) + "_t" + std::to_string(t) +
                              "_cp1";
          std::string name2 = "Poly" + std::to_string(n_poly) + "_face" +
                              std::to_string(i) + "_t" + std::to_string(t) +
                              "_cp2";
          std::string name3 = "Poly" + std::to_string(n_poly) + "_face" +
                              std::to_string(i) + "_t" + std::to_string(t) +
                              "_cp3";
          // std::cout << "Plane=" << i << "out of" << bb.rows() - 1 <<
          // std::endl; If b[t,0]==1, all the control points are in that
          // polytope

          polytopes_cons.push_back(m.addGenConstrIndicator(
              b[t][n_poly], 1, Acp0[i], GRB_LESS_EQUAL, bb[i], name0));
          polytopes_cons.push_back(m.addGenConstrIndicator(
              b[t][n_poly], 1, Acp1[i], GRB_LESS_EQUAL, bb[i], name1));
          polytopes_cons.push_back(m.addGenConstrIndicator(
              b[t][n_poly], 1, Acp2[i], GRB_LESS_EQUAL, bb[i], name2));
          polytopes_cons.push_back(m.addGenConstrIndicator(
              b[t][n_poly], 1, Acp3[i], GRB_LESS_EQUAL, bb[i], name3));
        }
      }
    }
  }
...
void SolverGurobi::setDC(double dc) { DC = dc; }
...
void SolverGurobi::setUseMinvo(bool use_minvo) { use_minvo_ = use_minvo; }
...
void SolverGurobi::setConstraintsX0() {
  ...
  for (int i = 0; i < 3; i++) {
    init_cons.push_back(m.addConstr(
        getPos(0, 0, i) == x0_[i],
        "InitialPosAxis_" +
            std::to_string(i))); // Initial position
                                 // std::cout << "Velocity" << std::endl;
    init_cons.push_back(
        m.addConstr(getVel(0, 0, i) == x0_[i + 3],
...
void SolverGurobi::setMaxConstraints() {
  Eigen::MatrixXd A_minvo_vel_ =
      basis_converter_.get_vel_bernstein_on_interval({0, dt_});
  Eigen::MatrixXd A_bezier_vel_ =
      basis_converter_.get_vel_minvo_on_interval({0, dt_});

  Eigen::MatrixXd minvo_vel_inv = A_minvo_vel_.inverse();
  std::vector<std::vector<double>> minvo_vel_inv_std =
      eigenMatrix2std(minvo_vel_inv);
  std::vector<std::vector<double>> bezier_vel_std =
      eigenMatrix2std(A_bezier_vel_);
  ...
  for (int t = 0; t < N_; t++) {
    ...
    for (int i = 0; i < 3; i++) {
      cvs.push_back({3 * (cp1[i] - cp0[i]) / dt_, 3 * (cp2[i] - cp1[i]) / dt_,
                     3 * (cp3[i] - cp2[i]) / dt_});
    }

    std::vector<std::vector<GRBLinExpr>> cvs_minvo = Multiply2Matrices(
        Multiply2Matrices(cvs, bezier_vel_std), minvo_vel_inv_std);
    // std::vector<GRBLinExpr> cv0_minvo = {cps_minvo[0][0], cps_minvo[1][0],
    // cps_minvo[2][0]}; std::vector<GRBLinExpr> cv1_minvo = {cps_minvo[0][1],
    // cps_minvo[1][1], cps_minvo[2][1]}; std::vector<GRBLinExpr> cv2_minvo =
    // {cps_minvo[0][2], cps_minvo[1][2], cps_minvo[2][2]};

    // add max vel constraints on minvo points
    // for(int cp = 0; cp < 3; ++cp){
    //   for(int axis = 0; axis < 3; ++axis){
    //     m.addConstr(cvs_minvo[axis][cp] <= v_max_, "MaxVel_t" +
    //     std::to_string(t) +
    //     "_axis_" + std::to_string(axis) + "_cp_" + std::to_string(cp));
    //     m.addConstr(cvs_minvo[axis][cp] >= -v_max_, "MinVel_t" +
    //     std::to_string(t) +
    //     "_axis_" + std::to_string(axis) + "_cp_" + std::to_string(cp));
    //   }
    // }
    ...
    for (int i = 0; i < 3; i++) {
      m.addConstr(getVel(t, 0, i) <= v_max_, "MaxVel_t" + std::to_string(t) +
                                                 "_axis_" + std::to_string(i));
      m.addConstr(getVel(t, 0, i) >= -v_max_, "MinVel_t" + std::to_string(t) +
                                                  "_axis_" + std::to_string(i));

      m.addConstr(getAccel(t, 0, i) <= a_max_,
                  "MaxAccel_t" + std::to_string(t) + "_axis_" +
                      std::to_string(i));
      m.addConstr(getAccel(t, 0, i) >= -a_max_,
                  "MinAccel_t" + std::to_string(t) + "_axis_" +
                      std::to_string(i));

      m.addConstr(getJerk(t, 0, i) <= j_max_, "MaxJerk_t" + std::to_string(t) +
                                                  "_axis_" + std::to_string(i));
      m.addConstr(getJerk(t, 0, i) >= -j_max_, "MinJerk_t" + std::to_string(t) +
                                                   "_axis_" +
                                                   std::to_string(i));
    }
  }
...
void SolverGurobi::setFactorInitialAndFinalAndIncrement(
    double factor_initial, double factor_final, double factor_increment) {
...
bool SolverGurobi::genNewTraj() {
  ...
  if (factor_initial_ < 1) {
    std::cout << "factor_initial_ is less than one, it doesn't make sense"
              << std::endl;
  }
  ...
  /*     i        = i - factor_increment_)*/
  {
    ...
    // std::cout << "timeout time is " << max_solver_time-runtime_s_ <<
    // std::endl;
    ...
    if (solved == true) // solved and Wmax is satisfied
    {
      ...
      // std::cout << "Factor= " << i << "(dt_= " << dt_ << ")---> worked" <<
      // std::endl;
    } else {
      // std::cout << "Factor= " << i << "(dt_= " << dt_ << ")---> didn't
      // worked" << std::endl;
    }
  }
...
void SolverGurobi::setVerbose(int verbose) {
  m.set("OutputFlag",
        std::to_string(verbose)); // 1 if you want verbose, 0 if not
}

void SolverGurobi::setWMax(double w_max) { w_max_ = w_max; }
  ...
  for (int t = 0; t < N_ - 1; t++) // From 0....N_-2
  {
    for (int i = 0; i < 3; i++) {
      dyn_cons.push_back(m.addConstr(getPos(t, dt_, i) == getPos(t + 1, 0, i),
                                     "ContPos_t" + std::to_string(t) + "_axis" +
                                         std::to_string(i))); // Continuity
                                                              // in position
      dyn_cons.push_back(m.addConstr(getVel(t, dt_, i) == getVel(t + 1, 0, i),
                                     "ContVel_t" + std::to_string(t) + "_axis" +
                                         std::to_string(i))); // Continuity
                                                              // in velocity
      dyn_cons.push_back(
...
bool SolverGurobi::isWmaxSatisfied() {
  for (int n = 0; n < N_; n++) {
    ...
    double w_desired =
        (denominator > 0.001) ? fabs(numerator / denominator) : 0.5 * w_max_;

    if (w_desired > w_max_) {
      std::cout << "w_desired > than w_max: " << w_desired << " > " << w_max_
                << "  , solving again" << std::endl;
      return false;
    }
  }
  return true;
...
bool SolverGurobi::callOptimizer() {
  ...
  // m.write(ros::package::getPath("faster") + "/models/model2_" +
  // std::to_string(temporal_) +
  // ".lp");
  ...
  auto elapsed =
      std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
          .count() /
      1000.0;
  // std::cout << "*************************Finished Optimization: " << elapsed
  // << " s" << std::endl; std::cout << "*************************Gurobi
  // RUNTIME: " << m.get(GRB_DoubleAttr_Runtime) << " s" << std::endl;

  runtime_s_ = runtime_s_ + m.get(GRB_DoubleAttr_Runtime);

  /*  times_log.open("/home/jtorde/Desktop/ws/src/acl-planning/faster/models/times_log.txt",
    std::ios_base::app); times_log << elapsed << "\n"; times_log.close();*/

  // printf("Going to check status");
  int optimstatus = m.get(GRB_IntAttr_Status);
  if (optimstatus == GRB_OPTIMAL) {
    // m.write(ros::package::getPath("faster") + "/models/model_wt" +
    // std::to_string(temporal_) + ".lp");

    /*    if (polytopes_cons.size() > 0)  // Print the binary matrix only if
       I've included the polytope constraints
        {
    ...
    if (optimstatus == GRB_INF_OR_UNBD) {
      // printf("GUROBI SOLUTION: Unbounded or Infeasible. Maybe too small
      // dt?\n");

      /*      m.computeIIS();  // Compute the Irreducible Inconsistent Subsystem
         and write it on a file
            m.write("/home/jtorde/Desktop/ws/src/acl-planning/cvx/models/model_rp"
         + std::to_string(temporal_) + "dt=" + std::to_string(dt_) + ".ilp");*/
    }
...
double SolverGurobi::getDTInitial() {
  ...
  */
  dt_initial =
      std::max({t_vx, t_vy, t_vz, t_ax, t_ay, t_az, t_jx, t_jy, t_jz}) / N_;
  if (dt_initial >
      10000) // happens when there is no solution to the previous eq.
  {
...
GRBLinExpr SolverGurobi::getVel(int t, double tau, int ii)
    const { // t is the segment, tau is the time inside a specific segment
            // (\in[0,dt], i is the axis)

  GRBLinExpr vel =
      3 * x[t][0 + ii] * tau * tau + 2 * x[t][3 + ii] * tau + x[t][6 + ii];
  return vel;
}

GRBLinExpr
SolverGurobi::getAccel(int t, double tau,
                       int ii) { // t is the segment, tau is the time inside a
                                 // specific segment(\in[0, dt], i is the axis)
...
GRBLinExpr
SolverGurobi::getJerk(int t, double tau,
                      int ii) { // t is the segment, tau is the time inside a
                                // specific segment (\in[0,dt], i is the axis)

  GRBLinExpr jerk =
      6 * x[t][0 + ii]; // Note that here tau doesn't appear (makes sense)
  return jerk;
}
...
SolverGurobi::getCP0(int t) // Control Point 0 of interval t
{                           // Control Point 0 is initial position
                            // std::cout << "Getting CP0" << std::endl;
  std::vector<GRBLinExpr> cp = {getPos(t, 0, 0), getPos(t, 0, 1),
                                getPos(t, 0, 2)};
  return cp;
}

std::vector<GRBLinExpr>
SolverGurobi::getCP1(int t) // Control Point 1 of interval t
{
...
std::vector<GRBLinExpr>
SolverGurobi::getCP2(int t) // Control Point 2 of interval t
{
...
std::vector<GRBLinExpr>
SolverGurobi::getCP3(int t) // Control Point 3 of interval t
{                           // Control Point 3 is end position
  std::vector<GRBLinExpr> cp = {getPos(t, dt_, 0), getPos(t, dt_, 1),
                                getPos(t, dt_, 2)};
  return cp;
}

GRBModel SolverGurobi::getModel() const { return m; }
