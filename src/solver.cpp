...
void SolverGurobi::setObjective()  // I need to set it every time, because the objective depends
                                   // on the xFinal
{
...
void SolverGurobi::fillX()
{
    ...
    // Force the final input to be 0 (I'll keep applying this input if when I arrive to the
    // final state I still haven't planned again).
...
void SolverGurobi::setPolytopesConstraints()
{
    ...
    // Remove previous binary variables  (They depend on the number of polytopes--> I can't
    // reuse them)
    ...
    if (polytopes_.size() > 0)  // If there are polytope constraints
    {
        ...
        for (int t = 0; t < N_ + 1; t++)
        {
            ...
            {
                GRBVar variable = m.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_BINARY,
        ...
        // std::cout << "NUMBER OF FACES of the first polytope=" << polytopes_[0].A().rows() <<
        // std::endl;
        ...
        for (int t = 0; t < N_;
             t++)  // Start in t=1 (because t=0 is already fixed with the initial condition)
        {
            ...
            if (use_minvo_)
            {
                ...
                std::vector<std::vector<GRBLinExpr>> cps_minvo =
                    Multiply2Matrices(Multiply2Matrices(cps, bezier_std), minvo_inv_std);
                 ...
                 n_poly++)  // Loop over the number of polytopes
            {
                ...
                for (int i = 0; i < bb.rows(); i++)
                {
                    std::string name0 = "Poly" + std::to_string(n_poly) + "_face" +
                                        std::to_string(i) + "_t" + std::to_string(t) + "_cp0";
                    std::string name1 = "Poly" + std::to_string(n_poly) + "_face" +
                                        std::to_string(i) + "_t" + std::to_string(t) + "_cp1";
                    std::string name2 = "Poly" + std::to_string(n_poly) + "_face" +
                                        std::to_string(i) + "_t" + std::to_string(t) + "_cp2";
                    std::string name3 = "Poly" + std::to_string(n_poly) + "_face" +
                                        std::to_string(i) + "_t" + std::to_string(t) + "_cp3";
...
void SolverGurobi::setConstraintsX0()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        init_cons.push_back(
            m.addConstr(getAccel(0, 0, i) == x0_[i + 6],
...
void SolverGurobi::setMaxConstraints()
{
    ...
    for (int t = 0; t < N_; t++)
    {
        ...
        std::vector<std::vector<GRBLinExpr>> cvs_minvo =
            Multiply2Matrices(Multiply2Matrices(cvs, bezier_vel_std), minvo_vel_inv_std);
        ...
        //     m.addConstr(cvs_minvo[axis][cp] <= v_max_, "MaxVel_t" + std::to_string(t) +
        //     "_axis_" + std::to_string(axis) + "_cp_" + std::to_string(cp));
        //     m.addConstr(cvs_minvo[axis][cp] >= -v_max_, "MinVel_t" + std::to_string(t) +
        //     "_axis_" + std::to_string(axis) + "_cp_" + std::to_string(cp));
        //   }
        // }
        ...
        for (int i = 0; i < 3; i++)
        {
            m.addConstr(getVel(t, 0, i) <= v_max_,
                        "MaxVel_t" + std::to_string(t) + "_axis_" + std::to_string(i));
            m.addConstr(getVel(t, 0, i) >= -v_max_,
                        "MinVel_t" + std::to_string(t) + "_axis_" + std::to_string(i));

            m.addConstr(getAccel(t, 0, i) <= a_max_,
                        "MaxAccel_t" + std::to_string(t) + "_axis_" + std::to_string(i));
            m.addConstr(getAccel(t, 0, i) >= -a_max_,
                        "MinAccel_t" + std::to_string(t) + "_axis_" + std::to_string(i));

            m.addConstr(getJerk(t, 0, i) <= j_max_,
                        "MaxJerk_t" + std::to_string(t) + "_axis_" + std::to_string(i));
            m.addConstr(getJerk(t, 0, i) >= -j_max_,
                        "MinJerk_t" + std::to_string(t) + "_axis_" + std::to_string(i));
        }
    }
}
...
void SolverGurobi::setFactorInitialAndFinalAndIncrement(double factor_initial,
                                                        double factor_final,
...
bool SolverGurobi::genNewTraj()
{
    ...
    /*     i        = i - factor_increment_)*/
    {
        ...
        {
            // std::cout << "Factor= " << i << "(dt_= " << dt_ << ")---> didn't worked" <<
            // std::endl;
        }
    }
...
void SolverGurobi::setThreads(int threads) { m.set("Threads", std::to_string(threads)); }
...
void SolverGurobi::findDT(double factor) { dt_ = factor * std::max(getDTInitial(), 2 * DC); }
...
void SolverGurobi::setDynamicConstraints()
{
    ...
    for (int t = 0; t < N_ - 1; t++)  // From 0....N_-2
    {
        for (int i = 0; i < 3; i++)
        {
            dyn_cons.push_back(m.addConstr(
                getPos(t, dt_, i) == getPos(t + 1, 0, i),
                "ContPos_t" + std::to_string(t) + "_axis" + std::to_string(i)));  // Continuity
                                                                                  // in position
            dyn_cons.push_back(m.addConstr(
                getVel(t, dt_, i) == getVel(t + 1, 0, i),
                "ContVel_t" + std::to_string(t) + "_axis" + std::to_string(i)));  // Continuity
                                                                                  // in velocity
            dyn_cons.push_back(
...
bool SolverGurobi::callOptimizer()
{
    ...
    // m.write(ros::package::getPath("faster") + "/models/model2_" + std::to_string(temporal_) +
    // ".lp");
    ...
    // std::cout << "*************************Finished Optimization: " << elapsed << " s" <<
    // std::endl; std::cout << "*************************Gurobi RUNTIME: " <<
    // m.get(GRB_DoubleAttr_Runtime) << " s" << std::endl;
    ...
    if (optimstatus == GRB_OPTIMAL)
    {
        ...
        /*    if (polytopes_cons.size() > 0)  // Print the binary matrix only if I've included
    ...
    else
    {
        ...
        if (optimstatus == GRB_INF_OR_UNBD)
        {
            ...
            /*      m.computeIIS();  // Compute the Irreducible Inconsistent Subsystem and write
               it on a file
