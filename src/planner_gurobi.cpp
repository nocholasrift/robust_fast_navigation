...
PlannerROS::PlannerROS(ros::NodeHandle &nh) {
  ...
  // this is actually number of polynomials in traj, not number of polytopes...
  ...
  // Timers
  /*safetyTimer  = nh.createTimer(ros::Duration(.1), &PlannerROS::safetyLoop,
   * this);*/
  goalTimer =
      nh.createTimer(ros::Duration(_dt / 2.0), &PlannerROS::goalLoop, this);
  controlTimer =
...
void PlannerROS::occlusionPointscb(
...
void PlannerROS::controlLoop(const ros::TimerEvent &) {
  ...
  **************************************/
  ...
  costmap_2d::Costmap2D cmap = *_costmap->getCostmap();
  int width = costmap.getSizeInCellsX();
  int height = costmap.getSizeInCellsY();
  double resolution = costmap.getResolution();
  double origin_x = costmap.getOriginX();
  double origin_y = costmap.getOriginY();
  // order them in terms of most permissive to least permissive
  std::vector<unsigned char> occupied_values = {
      costmap_2d::INSCRIBED_INFLATED_OBSTACLE, costmap_2d::LETHAL_OBSTACLE};
