...
// TODO: This function is the same as solvePolyOrder2 but with other name (weird
// conflicts...)
...
public:
  BasisConverter() {
    ...
    A_minvo_pos << -0.43020000000000, 0.45680000000000, -0.02700000000000,
        0.00040000000000, 0.83490000000000, -0.45680000000000,
        -0.79210000000000, 0.49960000000000, -0.83490000000000,
        -0.45680000000000, 0.79210000000000, 0.49960000000000, 0.43020000000000,
        0.45680000000000, 0.02700000000000, 0.00040000000000;

    A_minvo_vel = Eigen::MatrixXd(3, 3);
    A_minvo_vel << 0.3750, -0.4330, 0.1250, -0.7500, 0, 0.7500, 0.3750, 0.4330,
        0.1250;

    // roots of lambda polynomial
    roots_lambda = {Eigen::Vector2d(0.0309000000000, 1.000000000000),
  ...
  Eigen::MatrixXd
  get_pos_bernstein_on_interval(const std::vector<double> &int2) {
    ...
    GiNaC::ex u =
        (int1[1] - int1[0]) / (int2[1] - int2[0]) * (t - int2[0]) + int1[0];

    // Define Bernstein basis polynomials
    GiNaC::ex B1 = -(u - 1) * (u - 1) * (u - 1);
    GiNaC::ex B2 = 3 * u * (u - 1) * (u - 1);
    GiNaC::ex B3 = -3 * u * u * (u - 1);
    GiNaC::ex B4 = u * u * u;

    GiNaC::ex B1_poly = B1.expand();
    GiNaC::ex B2_poly = B2.expand();
    GiNaC::ex B3_poly = B3.expand();
    GiNaC::ex B4_poly = B4.expand();

    std::vector<GiNaC::ex> B_coeffs = {B1_poly, B2_poly, B3_poly, B4_poly};
    Eigen::MatrixXd A_conv(4, 4);

    for (int i = 0; i < B_coeffs.size(); ++i) {
      GiNaC::ex B = B_coeffs[i];
      for (int deg = 3; deg >= 0; --deg)
        A_conv(i, 3 - deg) =
            GiNaC::ex_to<GiNaC::numeric>(B.coeff(t, deg)).to_double();
    }
  ...
  Eigen::MatrixXd
  get_vel_bernstein_on_interval(const std::vector<double> &int2) {
    std::vector<double> int1 = {0, 1};
    ...
    std::vector<GiNaC::ex> B_coeffs = {B1_poly, B2_poly, B3_poly};
  ...
  Eigen::MatrixXd get_pos_minvo_on_interval(const std::vector<double> &int2) {
    ...
    std::vector<GiNaC::ex> T = {u * u * u, u * u, u, 1};

    Eigen::MatrixXd A_conv(4, 4);
    std::vector<GiNaC::ex> polynomials;
    for (int row = 0; row < A_minvo_pos.rows(); ++row) {
      ...
      for (int deg = 3; deg >= 0; --deg)
        A_conv(row, 3 - deg) = GiNaC::ex_to<GiNaC::numeric>(
                                   polynomials[row].expand().coeff(t, deg))
                                   .to_double();
    }
  ...
  Eigen::MatrixXd get_vel_minvo_on_interval(const std::vector<double> &int2) {
    ...
    for (int row = 0; row < A_minvo_vel.rows(); ++row) {
      ...
      for (int deg = 2; deg >= 0; --deg)
        A_conv(row, 2 - deg) = GiNaC::ex_to<GiNaC::numeric>(
                                   polynomials[row].expand().coeff(t, deg))
                                   .to_double();
    }
...
public:
  ...
  void setFactorInitialAndFinalAndIncrement(double factor_initial,
                                            double factor_final,
...
protected:
  ...
  std::vector<GRBConstr>
      at_least_1_pol_cons; // Constraints at least in one polytope
