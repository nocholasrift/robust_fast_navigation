...
class BasisConverter
{
   public:
    BasisConverter()
    {
        ...
        A_minvo_vel << 0.3750, -0.4330, 0.1250, -0.7500, 0, 0.7500, 0.3750, 0.4330, 0.1250;
    ...
    Eigen::MatrixXd get_pos_bernstein_on_interval(const std::vector<double>& int2)
    {
        ...
        GiNaC::ex u = (int1[1] - int1[0]) / (int2[1] - int2[0]) * (t - int2[0]) + int1[0];
        ...
        for (int i = 0; i < B_coeffs.size(); ++i)
        {
            ...
            for (int deg = 3; deg >= 0; --deg)
                A_conv(i, 3 - deg) = GiNaC::ex_to<GiNaC::numeric>(B.coeff(t, deg)).to_double();
        }
    ...
    Eigen::MatrixXd get_vel_bernstein_on_interval(const std::vector<double>& int2)
    {
        ...
        for (int i = 0; i < B_coeffs.size(); ++i)
        {
            ...
            for (int deg = 2; deg >= 0; --deg)
                A_conv(i, 2 - deg) = GiNaC::ex_to<GiNaC::numeric>(B.coeff(t, deg)).to_double();
        }
    ...
    Eigen::MatrixXd get_pos_minvo_on_interval(const std::vector<double>& int2)
    {
        ...
        for (int row = 0; row < A_minvo_pos.rows(); ++row)
        {
            ...
            for (int i = 0; i < T.size(); ++i) polynomials[row] += A_minvo_pos(row, i) * T[i];

            for (int deg = 3; deg >= 0; --deg)
                A_conv(row, 3 - deg) =
                    GiNaC::ex_to<GiNaC::numeric>(polynomials[row].expand().coeff(t, deg))
                        .to_double();
        }
    ...
    Eigen::MatrixXd get_vel_minvo_on_interval(const std::vector<double>& int2)
    {
        ...
        {
            polynomials.push_back(0);
            for (int i = 0; i < T.size(); ++i) polynomials[row] += A_minvo_vel(row, i) * T[i];
