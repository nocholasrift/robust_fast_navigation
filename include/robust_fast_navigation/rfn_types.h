...
public:
  ...
  RFNSegment(const std::array<Eigen::VectorXd, 4> &coeffs, double duration)
      : _x(coeffs), _duration(duration) {}
  ...
  Eigen::VectorXd getVel(double t) const {
    return 3 * _x[0] * t * t + 2 * _x[1] * t + _x[2];
  }

  Eigen::VectorXd getAcc(double t) const { return 6 * _x[0] * t + 2 * _x[1]; }

  Eigen::VectorXd getJerk(double t) const { return 6 * _x[0]; }

  Eigen::VectorXd getP0() const { return _x[3]; }

  Eigen::VectorXd getP1() const { return (_x[2] * _duration + 3 * _x[3]) / 3; }

  Eigen::VectorXd getP2() const {
    return (_x[1] * _duration * _duration + 2 * _x[2] * _duration + 3 * _x[3]) /
           3;
  }

  Eigen::VectorXd getP3() const { return getPos(_duration); }
...
public:
  ...
  // assumes each row of ctrl_pts is 1 cp...
  RFNTrajectory(const Eigen::MatrixXd &ctrl_pts, double dt) {
    buildSegmentsFromCPs(ctrl_pts, dt);
  }

  void buildSegmentsFromCPs(const Eigen::MatrixXd &ctrl_pts, double dt) {
    // this is a collection of cubic curves...
    // ensure ctrl point size is conssitent with that
    if (ctrl_pts.rows() % (kCubicDegree + 1) != 0) {
      throw std::runtime_error(
          "[RFNTrajectory] ctrl_pts rows size is not divisible by" +
          std::to_string(kCubicDegree + 1) + " (ctrl pts per segment)");
    }

    unsigned int n_segments = ctrl_pts.rows() / (kCubicDegree + 1);
    _segments.reserve(n_segments);

    double dt2 = dt * dt;
    double dt3 = dt2 * dt;

    // coeffs are a, b, c, d, and are defined per
    // axis: ax, bx, cx, dx, ay, by, ...
    // see https://arxiv.org/pdf/2001.04420
    // end of page 4
    for (unsigned int seg = 0; seg < n_segments; ++seg) {
      unsigned int cp_block = seg * (kCubicDegree + 1);
      Eigen::VectorXd cp0 = ctrl_pts.row(cp_block + 0);
      Eigen::VectorXd cp1 = ctrl_pts.row(cp_block + 1);
      Eigen::VectorXd cp2 = ctrl_pts.row(cp_block + 2);
      Eigen::VectorXd cp3 = ctrl_pts.row(cp_block + 3);

      // these coeffs are defined d[0] = dx, d[1] = dy...
      Eigen::VectorXd coeff_d = cp0;
      Eigen::VectorXd coeff_c = 3 * (cp1 - coeff_d) / dt;
      Eigen::VectorXd coeff_b =
          (3 * cp2 - 2 * coeff_c * dt - 3 * coeff_d) / dt2;
      Eigen::VectorXd coeff_a =
          (cp3 - coeff_b * dt2 - coeff_c * dt - coeff_d) / dt3;

      std::array<Eigen::VectorXd, kCubicDegree + 1> coeffs = {coeff_a, coeff_b,
                                                              coeff_c, coeff_d};
      _segments.emplace_back(coeffs, dt);
    }
  }
  ...
  Eigen::VectorXd getJerk(double t) const {
    int m = getSegment(t);
    double start_t = getSegmentStartTime(m);
  ...
  double getSegmentStartTime(int m) const {
    double t = 0.;
    for (int i = 0; i < m; ++i) {
  ...
  // supports differing segment times
  // if that is desireable in the future
...
template <typename T> struct GEVecBase {
  std::vector<T> x;
  GEVecBase() {}
...
auto operator+(const GEVecBase<T1> &lhs, const GEVecBase<T2> &rhs)
  ...
  if (rhs.x.size() != lhs.x.size() || rhs.x.size() == 0 || lhs.x.size() == 0) {
    throw std::runtime_error("GEVecBase: size mismatch " +
...
GEVecBase<T> GEVecBase<T>::operator+(const GVec &rhs) const {
  if (rhs.x.size() != x.size() || rhs.x.size() == 0 || x.size() == 0) {
    throw std::runtime_error("GVec: size mismatch " +
                             std::to_string(rhs.x.size()) +
