...
std::vector<GRBLinExpr> gurobiMatMul(const Eigen::MatrixXd& A, const std::vector<GRBLinExpr>& x)
{
    ...
    {
        GRBLinExpr exp = 0;
        for (int j = 0; j < x.size(); j++) exp += A(i, j) * x[j];
...
std::vector<std::vector<GRBLinExpr> > gurobiMatMul(
    const std::vector<std::vector<GRBLinExpr> >& A, const Eigen::MatrixXd& B)
{
    ...
    for (int i = 0; i < A.size(); i++)
    {
        ...
        {
            GRBLinExpr entry = 0;
            for (int k = 0; k < B.rows(); k++) entry += A[i][k] * B(k, j);
