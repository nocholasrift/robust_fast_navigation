...
getOccupied(const map_util::occupancy_grid_t &occ_grid) {
  ...
  for (unsigned int i = 0; i < width; i++) {
    for (unsigned int j = 0; j < height; j++) {
      ...
      if (occ_grid.is_occupied(i, j, "inflated")) {
        ...
        // std::cout << paddedObs.back().transpose() << "\t"
        //           << static_cast<int>(occ_grid.get_cost(i, j, "inflated"))
        //           << "\n";
      }
    }
  }
...
inline void shrinkPolytopes(std::vector<Eigen::MatrixX4d> &hpolys,
  ...
  for (auto &poly : hpolys) {
    int numHalfPlanes =
        poly.rows(); // Get the number of half-planes (rows in the matrix)
    ...
    for (int i = 0; i < numHalfPlanes; ++i) {
      ...
      for (int j = 0; j < N; ++j) {
        ...
        if (k == 0 && !isInPoly(poly, Eigen::Vector2d(initialPVAJ(0, 0),
                                                      initialPVAJ(1, 0)))) {
          poly(i, 3) -= step_size;
          break;
        }
        if (k == hpolys.size() - 1 &&
            !isInPoly(poly,
                      Eigen::Vector2d(finalPVAJ(0, 0), finalPVAJ(1, 0)))) {
          poly(i, 3) -= step_size;
          break;
        }
      }
...
inline bool createCorridorJPS(const std::vector<Eigen::Vector2d> &path,
  ...
  if (!isInPoly(polys[0],
                Eigen::Vector2d(initialPVAJ(0, 0), initialPVAJ(1, 0)))) {
    std::cout << "[Corridor] Start was not in poly, adding extra polygon to "
                 "correct this\n";
    std::vector<Eigen::VectorXd> poses = {initialPVAJ.col(0),
                                          initialPVAJ.col(0)};
    std::vector<Eigen::MatrixX4d> tmp;
    status = convexCover(poses, obs3d, Eigen::Vector3d(x, y, -.1),
                         Eigen::Vector3d(x + w, y + h, .1), .1, 5.0, tmp);
    if (status)
      polys.insert(polys.begin(), tmp[0]);
  }

  if (!isInPoly(polys.back(),
                Eigen::Vector2d(finalPVAJ(0, 0), finalPVAJ(1, 0)))) {
    std::cout << "[Corridor] End was not in poly, adding extra polygon to "
                 "correct this"
              << std::endl;
    ;
  ...
  for (Eigen::Vector3d ob : obs3d) {
    ...
    for (Eigen::MatrixX4d poly : polys) {
      if (isInPoly(poly, ob.head(2))) {
        /*std::cout << "val: " << _map._map.atPosition("obstacles",
         * ob.head(2))*/
        /*          << std::endl;*/
        // std::cout << "cost at " << ob.transpose() << " is "
        //           << static_cast<int>(_map.get_cost(ob(0), ob(1),
        //           "obstacles"))
        //           << std::endl;
