...
/**********************************************************************
    ...
    Returns:
      - Expanded polygon
***********************************************************************/
inline Eigen::MatrixX4d expandPoly(const Eigen::MatrixX4d &poly, const double &d)
{
    ...
    for (int i = 0; i < ret.rows(); i++)
    {
        ...
        Eigen::Vector3f p;
        if (n[0] < 1e-6 && n[1] < 1e-6 && n[2] < 1e-6) continue;
...
/**********************************************************************
    ...
    inside polygon?
***********************************************************************/
inline double dist2Poly(const Eigen::Vector2d &point, const Eigen::MatrixX4d &poly)
{
    double dist = 100000.;
    ...
    for (int i = 0; i < poly.rows(); i++)
    {
        ...
        if (plane_dist < dist) dist = plane_dist;
    }
...
***********************************************************************/
inline bool isTrajOutsidePolys(const trajectory_msgs::JointTrajectory &traj,
                               const std::vector<Eigen::MatrixX4d> &hpolys, double thresh)
{
    ...
    for (const trajectory_msgs::JointTrajectoryPoint &pt : traj.points)
    {
        ...
        if (max_dist > thresh) return true;
    }
...
***********************************************************************/
inline bool isTrajOverlappingObs(const trajectory_msgs::JointTrajectory &traj,
...
inline bool testMap(unsigned char val, unsigned char test_val)
{
...
inline void bresenham(const costmap_2d::Costmap2D &_map, unsigned int abs_da,
...
inline bool lineSphereIntersection(const Eigen::Vector2d &A, const Eigen::Vector2d &B,
...
inline bool getJPSIntersectionWithSphere(const std::vector<Eigen::Vector2d> &path,
...
inline Eigen::MatrixX2d getVertices(const Eigen::MatrixX4d &poly)
{
...
inline bool reparam_traj(std::vector<double> &ss, std::vector<double> &xs,
                         std::vector<double> &ys, const trajectory_msgs::JointTrajectory &traj)
{
