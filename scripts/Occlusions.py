...
marker_pub = marker_pub1 = pose_arr_pub = None
...
def calcOcclusion(window, pos, a, b):
    ...
    scale = math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2) / 1.5
...
def main():
    global marker_pub, marker_pub1, pose_arr_pub, JACKAL_LENGTH, JACKAL_WIDTH, occ_man, jackal_pos, angles
    ...
    marker_pub = rospy.Publisher(
        "occlusions", MarkerArray, queue_size=1000
    )

    pose_arr_pub = rospy.Publisher("/occ_points", PoseArray, queue_size=1000, latch=True)
