...
void PlannerROS::spin() {
  std::string costmap_name =
      _use_global_costmap ? "global_costmap" : "local_costmap";
  ...
  _costmap_thread =
      std::make_unique<std::thread>([this]() { _costmap_executor->spin(); });
  ...
  RCLCPP_INFO(this->get_logger(), "Planner spinning with %s",
              costmap_name.c_str());
...
void PlannerROS::clickedPointcb(
  ...
  RCLCPP_INFO(this->get_logger(),
              "New goal received from clicked point: [%.2f, %.2f]",
              _goal_vec(0), _goal_vec(1));
}

void PlannerROS::mpcHorizoncb(
  ...
  if (mpcHorizon.points.size() >= 2) {
    ...
    double t1 =
        rclcpp::Duration(mpcHorizon.points[1].time_from_start).seconds();
    double t0 =
        rclcpp::Duration(mpcHorizon.points[0].time_from_start).seconds();

    _mpc_dt = t1 - t0;
  } else {
    RCLCPP_WARN(this->get_logger(),
                "MPC Horizon received with insufficient points (%zu)",
...
void PlannerROS::goalcb(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
  ...
  RCLCPP_WARN(this->get_logger(), "goal received: x=%.2f, y=%.2f", _goal_vec(0),
              _goal_vec(1));
}
...
void PlannerROS::publishOccupied() {
  ...
  for (const auto &p : padded) {
    // ROS 2 messages use standard vectors; emplace_back returns a reference in
    // C++17+
...
void PlannerROS::odomcb(const nav_msgs::msg::Odometry::SharedPtr msg) {
  ...
  if (_is_barn && !_is_goal_set) {
    ...
    RCLCPP_INFO(this->get_logger(), "goal is %.4f\t%.4f", _goal_vec(0),
                _goal_vec(1));
  }
...
void PlannerROS::lasercb(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
  ...
  for (size_t i = 0; i < msg->ranges.size(); i++) {
    ...
    double angle = msg->angle_min +
                   static_cast<double>(i) * msg->angle_increment + _odom(2);
...
void PlannerROS::visualizeTraj() {
  RCLCPP_INFO(this->get_logger(), "in visualize traj: %lu",
              sentTraj.points.size());
  if (!sentTraj.points.empty()) {
    if (!_use_arclen) {
      ...
      if (states.empty()) {
        RCLCPP_WARN(this->get_logger(),
                    "arc length reparameterization failed!");
        return;
      }
      ...
      RCLCPP_WARN(this->get_logger(), "arclenmsg points sz: %lu",
                  arclenmsg.points.size());
...
void PlannerROS::controlLoop() {
  ...
  // Check goal proximity (using _goal_vec established in clickedPoint/odom
  // callbacks)
  ...
  **************************************/
  ...
  RCLCPP_INFO(this->get_logger(), "Occupancy grid created in %.4f seconds",
              grid_time);
  ...
  **************************************/
  ...
  RCLCPP_INFO(this->get_logger(), "horizon is %.4f", _curr_horizon);
}
...
bool PlannerROS::plan(bool is_failsafe) {
  ...
  RCLCPP_INFO(this->get_logger(), "MPC horizon size is %zu",
              mpcHorizon.points.size());

  Eigen::MatrixXd initialPVAJ(3, 4);
  Eigen::MatrixXd finalPVAJ(3, 4);

  // goal(0) etc. refers to the Eigen::VectorXd established in
  // odomcb/clickedPointcb
  finalPVAJ << Eigen::Vector3d(_goal_vec(0), _goal_vec(1), 0),
  ...
  if (sentTraj.points.empty() || _is_teleop || _is_goal_reset ||
    ...
    initialPVAJ << Eigen::Vector3d(_odom(0), _odom(1), 0),
        ...
        Eigen::Vector3d::Zero();
    RCLCPP_INFO(this->get_logger(),
                "initializing PVAJ where sentTraj is 0, mpchorizon is empty");
    ...
    initialPVAJ.col(2) = Eigen::Vector3d(p.accelerations[0], p.accelerations[1],
                                         p.accelerations[2]);
  ...
  **************************************/
  ...
  RCLCPP_INFO(
      this->get_logger(), "is start occupied? %d",
      _occ_grid->is_occupied(_odom(0), _odom(1), map_util::Layer::kInflated));
  ...
  RCLCPP_INFO(this->get_logger(), "planner finished in %.4f with status %d",
              (this->now() - before).seconds(),
              static_cast<int>(_prev_plan_status));

  if (_prev_plan_status != PlannerStatus::SUCCESS) {
    RCLCPP_WARN(this->get_logger(),
                "Planner failed to find path. Conditions: V:%.2f A:%.2f J:%.2f",
                initialPVAJ.col(1).norm(), initialPVAJ.col(2).norm(),
                initialPVAJ.col(3).norm());
  }

  ros2_utils::visualizePolytope(hPolys, meshPub, edgePub, this->now(),
                                _frame_str);
  ...
  p_start.velocities = {initialPVAJ(0, 1), initialPVAJ(1, 1),
                        initialPVAJ(2, 1)};
  p_start.accelerations = {initialPVAJ(0, 2), initialPVAJ(1, 2),
                           initialPVAJ(2, 2)};
  ...
  if (_use_arclen) {
    ...
    if (mpcHorizon.points.empty()) {
      ...
      for (int i = startInd; i < trajInd; i++) {
        ...
        aTraj.points.back().time_from_start =
            rclcpp::Duration::from_seconds((i - startInd) * _mpc_dt);
      }
...
trajectory_msgs::msg::JointTrajectory PlannerROS::convertTrajToMsg(
    const std::vector<rfn_state_t> &trajectory, double traj_dt,
    ...
    const rclcpp::Time &current_time) // We need to pass the time in ROS 2
{
  ...
  msg.header.frame_id =
      std::string(frame_str); // Convert string_view to std::string
